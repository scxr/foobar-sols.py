...
def multiply_row_of_square_matrix(m, row, k):
    ...
    return multi_matrix(row_operator, m)
...
def add_multiple_of_row_of_square_matrix(m, source_row, k, target_row):
    # add k * source_row to target_romakeneww of matrix m
    n = len(m)
    row_operator = make_identity(n)
...
def answer(m):
    ...
    result = multi_matrix(invert_matrix(subtract_matrices(make_identity(len(q)), q)), r)
